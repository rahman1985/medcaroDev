...
class FW_Ext_Builder_Templates_Component_Column extends FW_Ext_Builder_Templates_Component
{
	...
	{
		return esc_html__('Columns', 'docdirect');
	}
	...
	public function _render($data)
	{
		...
		if (empty($html)) {
			$html = '<div class="fw-text-muted">'. esc_html__('No Templates Saved', 'docdirect') .'</div>';
		} else {
			$html =
				'<p class="fw-text-muted load-template-title">'. esc_html__('Load Template', 'docdirect') .':</p>'
	...
	public function _enqueue($data)
	{
		...
		wp_localize_script(
			...
			array(
				'l10n' => array(
					'template_name' => esc_html__('Template Name', 'docdirect'),
					'save_template' => esc_html__('Save Column', 'docdirect'),
					'save_template_tooltip' => esc_html__('Save as Template', 'docdirect'),
				),
			)
		);
	}
	...
	public function _action_ajax_save_template()
	{
		...
		if (empty($template['title'])) {
			$template['title'] = esc_html__('No Title', 'docdirect');
		}
