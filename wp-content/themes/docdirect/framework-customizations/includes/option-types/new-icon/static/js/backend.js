jQuery(function ($) {
	var optionTypeClass = '.fw-option-type-new-icon';
	...
	fwEvents.on('fw:options:init', function (data) {
		...
		$options.find('.js-option-type-new-icon-item').click(function () {
		...
		$options.find('.js-option-type-new-icon-dropdown')
			.on('change', function () {
				var $this = $(this);
				var group = $this.val();

				$this.closest(optionTypeClass).find('.js-option-type-new-icon-item').each(function () {
