...
class WCFMvm {
    ...
    add_action( 'wcfm_membership_data_reset', array( &$this, 'wcfmvm_disable_vendor' ) );
	...
	function wcfmvm_vendor_membership_expire( $member_id, $wcfm_membership_id = 0 ) {
		...
		if( $member_id ) {
			...
			if( $wcfm_membership_id && wcfm_is_valid_membership( $wcfm_membership_id ) ) {
				...
				$expire_product_status = isset( $membership_expire_rules['product_status'] ) ? $membership_expire_rules['product_status'] : 'draft';
