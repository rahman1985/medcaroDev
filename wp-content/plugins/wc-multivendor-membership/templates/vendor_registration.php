				...
				do_action( 'begin_wcfm_membership_registration_form' );
				 if( $user_id ) {
					...
					if( apply_filters( 'wcfm_is_allow_sms_verification', true ) ) {
						if( $sms_verification ) {
							...
							if( $user_id && $sms_verified ) {
								...
								?>
								<div class="wcfm_sms_verified">
									<input type="number" name="wcfm_sms_verified_input" data-required="1" data-required_message="<?php _e( 'Phone Verification Code: ', 'wc-multivendor-membership' ) . _e( 'This field is required.', 'wc-frontend-manager' ); ?>" class="wcfm-text wcfm_sms_verified_input" placeholder="<?php _e( 'OTP', 'wc-multivendor-membership' ); ?>" value="" />
					...
					if( $WCFM->is_marketplace && ($WCFM->is_marketplace == 'wcfmmarketplace') ) {
						global $WCFMmp;
						if( !apply_filters( 'wcfm_is_allow_store_name', true ) || !$WCFMmp->wcfmmp_vendor->is_vendor_sold_by() ) {
