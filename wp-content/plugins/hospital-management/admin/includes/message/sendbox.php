 	...
 	<table class="table">
 		<thead>
 			<tr>
               ...
               $offest_value = ($p-1) * $max;
               	echo $obj_message->hmgt_pagination($totlal_message,$p,$prev,$next,'page=hmgt_message&tab=sentbox');
 		...
 		<tbody>
			...
			foreach($message as $msg_post)
			{
				...
				<td><a href="?page=hmgt_message&tab=view_message&from=sendbox&id=<?php echo  $msg_post->ID;?>"><?php echo wordwrap($msg_post->post_title,10,"<br>\n",TRUE);?><?php if($obj_message->hmgt_count_reply_item($msg_post->ID)>=1){?><span class="badge badge-success pull-right"><?php echo $obj_message->hmgt_count_reply_item($msg_post->ID);?></span><?php } ?></a></td>
				 <td>
					  <?php 					
