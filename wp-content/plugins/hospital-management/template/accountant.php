...
$active_tab=isset($_REQUEST['tab'])?$_REQUEST['tab']:'accountantlist'; 
//access right
...
if(isset($_POST['save_accountant']))
{
	if(isset($_FILES['upload_user_avatar_image']) && !empty($_FILES['upload_user_avatar_image']) && $_FILES['upload_user_avatar_image']['size'] !=0)
	{
		...
		{
		 $accountant_image=MJ_hmgt_load_documets($_FILES['upload_user_avatar_image'],'upload_user_avatar_image','pimg');
			$accountant_image_url=content_url().'/uploads/hospital_assets/'.$accountant_image;
		}
		else 
		{
			$accountant_image=$_REQUEST['hidden_upload_user_avatar_image'];
			$accountant_image_url=$accountant_image;
		}
	...
	{
	 $ext=MJ_hmgt_check_valid_extension($accountant_image_url);
		 if(!$ext == 0)
			{			 
				$result=$user_object->hmgt_add_user($_POST);
				$returnans=update_user_meta( $result,'hmgt_user_avatar',$accountant_image_url);
				if($result)
				{
					wp_redirect ( home_url().'?dashboard=user&page=accountant&tab=accountantlist&message=2');
				}
			}
			else{ ?>
	...
	else
	{
		if( !email_exists( $_POST['email'] ) && !username_exists( $_POST['username'] ))
		{
			...
			if(!$ext == 0)
			{
				...
				if($result)
				{
					wp_redirect ( home_url().'?dashboard=user&page=accountant&tab=accountantlist&message=1');
				}
			}
			else{ ?>
			<div id="message" class="updated below-h2">
...
if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')
{
	...
	$result=$user_object->delete_usedata(MJ_hmgt_id_decrypt($_REQUEST['accountant_id']));
	if($result)
	{
		wp_redirect ( home_url().'?dashboard=user&page=accountant&tab=accountantlist&message=3');
	}
}
...
<div class="panel-body panel-white"><!-- START PANEL BODY DIV -->	
	 <ul class="nav nav-tabs panel_tabs" role="tablist">
		   <li class="<?php if($active_tab=='accountantlist'){?>active<?php }?>">
			  <a href="?dashboard=user&page=accountant&tab=accountantlist" class="tab <?php echo $active_tab == 'accountantlist' ? 'active' : ''; ?>" >
				 <i class="fa fa-align-justify"></i> <?php _e('Accountant List', 'hospital_mgt'); ?></a>
			  </a>
		  </li>
		  <li class="<?php if($active_tab=='addaccountant'){?>active<?php }?>">
		  <?php  if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit')
			{?>
				<a href="?dashboard=user&page=accountant&tab=addaccountant&&action=edit&accountant_id=<?php echo $_REQUEST['accountant_id'];?>" class="tab <?php echo $active_tab == 'addaccountant' ? 'active' : ''; ?>">
				<i class="fa fa"></i> <?php _e('Edit Accountant', 'hospital_mgt'); ?></a>
			 <?php 
			}
			else
			{
				...
				{			
				?>				
					<a href="?dashboard=user&page=accountant&tab=addaccountant&&action=insert" class="tab <?php echo $active_tab == 'addaccountant' ? 'active' : ''; ?>">
					<i class="fa fa-plus-circle"></i> <?php _e('Add Accountant', 'hospital_mgt'); ?></a>
				<?php
				}
			}
			?>	  
	...
	<!-- START PANEL BODY DIV -->
		<div class="panel-body">
            <div class="table-responsive"><!-- START TABLE RESPONSIVE DIV -->	
			    <table id="accountant_list" class="display dataTable " cellspacing="0" width="100%"><!-- START ACCOUNT LIST TABLE -->	
					<thead>
						<tr>
					...
					if(!empty($receptionistdata))
					{
						...
						<tr>
						   ...
						   <!-- <td class="department"><?php 
							...
							{
							?>
								<a href="?dashboard=user&page=accountant&tab=addaccountant&&action=edit&accountant_id=<?php echo MJ_hmgt_id_encrypt($retrieved_data->ID);?>" class="btn btn-info"> <?php _e('Edit', 'hospital_mgt' ) ;?></a>
							<?php
							}
							if($user_access['delete']=='1')
							{
							?>	
								<a href="?dashboard=user&page=accountant&tab=accountantlist&action=delete&accountant_id=<?php echo MJ_hmgt_id_encrypt($retrieved_data->ID);?>" class="btn btn-danger" 
				...
				</table><!-- END ACCOUNTANT LIST TABLE -->	
...
jQuery(document).ready(function($) {
	...
	if(isset($_REQUEST['accountant_id']))
		$accountant_id=MJ_hmgt_id_decrypt($_REQUEST['accountant_id']);
	$edit=0;
		if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit'){
    ...
    <div class="panel-body"><!-- START PANEL BODY DIV -->	
        <form name="accountant_form" action="" method="post" class="form-horizontal" id="accountant_form" enctype="multipart/form-data"><!-- START ACCOUNTANT FORM -->	
		...
		<div class="col-sm-offset-2 col-sm-8">
        	<input type="submit" value="<?php if($edit){ _e('Save Accountant','hospital_mgt'); }else{ _e('Add Accountant','hospital_mgt');}?>" name="save_accountant" class="btn btn-success"/>
        </div>
        </form><!-- END ACCOUNTANT FORM -->	
    </div><!-- END PANEL BODY DIV -->		        
  <?php 
}	 
?>	
