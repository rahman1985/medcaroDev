...
if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')
{
	...
	$result=$user_object->delete_usedata(MJ_hmgt_id_decrypt($_REQUEST['receptionist_id']));
	if($result)
	{
    ...
    <ul class="nav nav-tabs panel_tabs" role="tablist">
        <li class="<?php if($active_tab=='receptionistlist'){?>active<?php }?>">
          <a href="?dashboard=user&page=supportstaff&tab=receptionistlist" class="tab <?php echo $active_tab == 'receptionistlist' ? 'active' : ''; ?>" >
             <i class="fa fa-align-justify"></i> <?php _e('Support Staff List', 'hospital_mgt'); ?></a>
          </a>
        </li>
	   ...
	   <?php  if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit')
		{?>
			...
			<i class="fa fa"></i> <?php _e('Edit Support Staff', 'hospital_mgt'); ?></a>
		 <?php 
		}
		else
		{
			...
			?>				
				...
				<i class="fa fa-plus-circle"></i> <?php _e('Add New Support Staff', 'hospital_mgt'); ?></a>
			<?php
			}
		}
		?>	  
						   ...
						   <tbody>
							...
							if(!empty($receptionistdata))
							{
								...
								<tr>
									...
									{
									?>
										<a href="?dashboard=user&page=supportstaff&tab=addreceptionist&&action=edit&receptionist_id=<?php echo MJ_hmgt_id_encrypt($retrieved_data->ID);?>" class="btn btn-info"> <?php _e('Edit', 'hospital_mgt' ) ;?></a>
									<?php
									}
									if($user_access['delete']=='1')
									{
									?>	
										<a href="?dashboard=user&page=supportstaff&tab=receptionistlist&action=delete&receptionist_id=<?php echo MJ_hmgt_id_encrypt($retrieved_data->ID);?>" class="btn btn-danger" 
	...
	if($active_tab == 'addreceptionist')
	{
        ...
        <div class="panel-body"><!-- START PANEL BODY DIV-->
			<form name="nurse_form" action="" method="post" class="form-horizontal" id="nurse_form" enctype="multipart/form-data">	<!-- START STAFF FORM-->
				...
				<input type="hidden" name="user_id" value="<?php echo $receptionist_id;?>"  />
				<div class="header">	
				...
				<div class="col-sm-offset-2 col-sm-8">
					<input type="submit" value="<?php if($edit){ _e('Save','hospital_mgt'); }else{ _e('Save','hospital_mgt');}?>" name="save_receptionist" class="btn btn-success"/>
				</div>
			</form><!-- END STAFF FORM-->
        </div><!-- END PANEL BODY DIV-->
     <?php 
