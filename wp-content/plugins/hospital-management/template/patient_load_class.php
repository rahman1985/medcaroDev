				...
				else if($j== 1)
				{
					...
					$user_image = '';
							$userimage=get_user_meta($uid, 'hmgt_user_avatar', true);
								if(empty($userimage))
									{
										$user_image = '<img src='.get_option( 'hmgt_patient_thumb' ).' height="50px" width="50px" class="img-circle" />';
									}
							else
							$user_image= '<img src='.$userimage.' height="50px" width="50px" class="img-circle"/>';
					$row[ $column['dt'] ] = $user_image;
				}
				else if($j== 5)
				{
					$uid=$data[$i][ $columns[1]['db'] ];
					//$userimage=get_user_meta($uid, 'mobile', true);
					$html = '';
					$row[ $column['dt'] ] =get_user_meta($uid, 'mobile', true);;
				}
				else if($j== 6)
				{
					$uid=$data[$i][ $columns[1]['db'] ];
					$html = '';
					$html .= '<a  href="#" class="show-popup btn btn-default" idtest="'.$uid.'"><i class="fa fa-eye"></i> View Detail</a>
				<a  href="#" class="show-charges-popup btn btn-default" idtest="'.$uid.'">
				<i class="fa fa-money"></i>Charges</a>
				<a href="?dashboard=user&page=patient&tab=addpatient&action=edit&patient_id='.$uid.'" class="btn btn-info">Edit</a>
               ';
					$row[ $column['dt'] ] =$html;
				}
...
AND wp_usermeta.meta_value = \'inpatient\'
)
