...
if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')
	{
		...
		$result=$user_object->delete_usedata(MJ_hmgt_id_decrypt($_REQUEST['laboratorist_id']));
		if($result)
		{
	 ...
	 <ul class="nav nav-tabs panel_tabs" role="tablist">
		   <li class="<?php if($active_tab=='laboratoristlist'){?>active<?php }?>">
			  <a href="?dashboard=user&page=laboratorystaff&tab=laboratoristlist" class="tab <?php echo $active_tab == 'laboratoristlist' ? 'active' : ''; ?>" >
				 <i class="fa fa-align-justify"></i> <?php _e('Laboratory Staff List', 'hospital_mgt'); ?></a>
			  </a>
		  </li>
		  ...
		  <?php  if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit')
			{?>
				...
				<i class="fa fa"></i> <?php _e('Edit Laboratory Staff', 'hospital_mgt'); ?></a>
			 <?php 
			}
			else
			{
				...
				?>				
					...
					<i class="fa fa-plus-circle"></i> <?php _e('Add New Laboratory Staff', 'hospital_mgt'); ?></a>
				<?php
				}
			}
			?>	  
				...
				if(!empty($receptionistdata))
				{
				 ...
				 ?>
					<tr>
						...
						{
						?>
							<a href="?dashboard=user&page=laboratorystaff&tab=addlaboratorist&&action=edit&laboratorist_id=<?php echo MJ_hmgt_id_encrypt($retrieved_data->ID);?>" class="btn btn-info"> <?php _e('Edit', 'hospital_mgt' ) ;?></a>
						<?php
						}
						if($user_access['delete']=='1')
						{
						?>	
							<a href="?dashboard=user&page=laboratorystaff&tab=laboratoristlist&action=delete&laboratorist_id=<?php echo MJ_hmgt_id_encrypt($retrieved_data->ID);?>" class="btn btn-danger" 
                     ...
                     <div id="upload_user_avatar_preview" >
	                     <?php if($edit) 
	                     	...
	                     	<?php }
	                     		...
	                     		?>
					        <img class="image_preview_css" style="max-width:100%;" src="<?php if($edit)echo esc_url( $user_info->hmgt_user_avatar ); ?>" />
					        <?php 
	                     	}
	                     	}
					        else {
