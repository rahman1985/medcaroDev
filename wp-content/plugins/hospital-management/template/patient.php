	...
	if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')
		{
			...
			$result=$user_object->delete_usedata(MJ_hmgt_id_decrypt($_REQUEST['patient_id']));
					...
					<tbody>
						...
						if(!empty($patientlist))
						{
							...
							<tr>
								...
								{
								?>	
									<a href="?dashboard=user&page=patient&tab=addpatient&action=edit&patient_id=<?php echo MJ_hmgt_id_encrypt($retrieved_data->ID);?>" class="btn btn-info"> <?php _e('Edit', 'hospital_mgt' );?></a>
								<?php 
								}
								if($user_access['delete']=='1')
								{
								?>	
									<a href="?dashboard=user&page=patient&tab=patientlist&action=delete&patient_id=<?php echo MJ_hmgt_id_encrypt($retrieved_data->ID);?>" class="btn btn-danger" onclick="return confirm('<?php _e('Are you sure you want to delete this record?','hospital_mgt');?>');"><?php _e('Delete', 'hospital_mgt' );?></a>
								<?php 
								}
								?>                
								</td>
...
<?php }
        ...
        <div class="panel-body"><!-- START PANEL BODY DIV-->
			<form name="patient_form" action="" method="post" class="form-horizontal" id="patient_form" enctype="multipart/form-data"><!-- START OUTPATIENT FORM-->
				...
				<input type="hidden" name="user_id" value="<?php if(isset($_REQUEST['patient_id'])) echo MJ_hmgt_id_decrypt($_REQUEST['patient_id']);?>"  />
				<div class="header">	
				...
				<div class="form-group">
					...
					<label class="col-lg-2 col-md-2 col-sm-2 col-xs-12 control-label" for="blood_group"><?php _e('Blood Group','hospital_mgt');?><span class="require-field">*</span></label>
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 has-feedback">
					...
					<div class="col-sm-offset-2 col-sm-8">
						 <div id="upload_user_avatar_preview" >
							 <?php if($edit) 
								...
								{?>
								<img class="imagen image_preview_css" alt="" src="<?php echo get_option( 'hmgt_patient_thumb' ); ?>">
								<?php }
								else {
									
									?>
								<img  class="imagen image_preview_css" src="<?php if($edit)echo esc_url( $user_info->hmgt_user_avatar ); ?>" />
								<?php 
								}
								}
								else {
     ...
     <?php } 
		...
		if(isset($_POST['save_patient_step2']))
		{
		       ...
		       $guardian_data=array('guardian_id'=>$_POST['guardian_id'],
						...
						'image'=>MJ_hmgt_strip_tags_and_stripslashes($patient_image_url),
			...
			<form name="guardian_form" action="" method="post" class="form-horizontal" id="guardian_form" enctype="multipart/form-data"><!-- START GUARDIAN FORM-->
				...
				<div class="form-group">
					...
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 has-feedback">
						...
						value="<?php if($edit){ if(isset($user_info['guardian_id'])) { print $user_info['guardian_id']; } } elseif(isset($_POST['guardian_id'])) echo $_POST['guardian_id'];?>"  <?php if($edit) echo "readonly";?> name="guardian_id">
					</div>
					<label class="col-lg-2 col-md-2 col-sm-2 col-xs-12 control-label" for="first_name"><?php _e('First Name','hospital_mgt');?><span class="require-field">*</span></label>
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 has-feedback">
						<input id="first_name" class="form-control validate[required,custom[onlyLetter_specialcharacter]] text-input" type="text" maxlength="50" value="<?php if($edit){ if(isset($user_info['first_name'])) { echo $user_info['first_name'];} }elseif(isset($_POST['first_name'])) echo $_POST['first_name'];?>" name="first_name">
					</div>
				</div>
				<div class="form-group">
					<label class="col-lg-2 col-md-2 col-sm-2 col-xs-12 control-label" for="middle_name"><?php _e('Middle Name','hospital_mgt');?></label>
				...
				<div class="form-group">
					...
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 has-feedback">
						<input id="guardian_realtion" class="form-control validate[required,custom[onlyLetter_specialcharacter]] text-input" maxlength="30" type="text"  name="guardian_realtion" 
				...
				<div class="form-group">
					<label class="col-lg-2 col-md-2 col-sm-2 col-xs-12 control-label" for="state_name"><?php _e('State','hospital_mgt');?></label>
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 has-feedback">
						<input id="city_name" class="form-control validate[custom[city_state_country_validation]]" maxlength="50" type="text"  name="state_name" 
				...
				<div class="form-group">
					<label class="col-lg-2 col-md-2 col-sm-2 col-xs-12 control-label" for="mobile"><?php _e('Mobile Number','hospital_mgt');?></label>
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 has-feedback">
				...
				<div class="form-group">
					<label class="col-sm-2 control-label" for="photo"><?php _e('Image','hospital_mgt');?></label>				
						<div class="col-sm-3">
							<input type="hidden" id="hmgt_user_avatar_url" class="form-control" name="hmgt_user_avatar_url"  
						value="<?php if($edit)  if(isset($user_info['image'])) echo esc_url( $user_info['image'] );elseif(isset($_POST['upload_user_avatar_image'])) echo $_POST['upload_user_avatar_image']; ?>" readonly />
							<input type="hidden" name="hidden_upload_user_avatar_image" value="<?php if($edit){  if(isset($user_info['image'])) { echo $user_info['image'];} }elseif(isset($_POST['upload_user_avatar_image'])) echo $_POST['upload_user_avatar_image'];?>">
							 <input id="upload_user_avatar_image" name="upload_user_avatar_image" type="file" class="form-control file" value="<?php _e( 'Upload image', 'hospital_mgt' ); ?>" />
				...
				<form name="admit_form" action="" method="post" class="form-horizontal" id="admit_form"><!-- START PATIENT ADMIT FORM-->
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							...
							value="<?php if($edit){  if(isset($user_info['admit_date']))  { if(!empty($user_info['admit_date']!='0000-00-00')) { echo date(MJ_hmgt_date_formate(),strtotime($user_info['admit_date'])); } } }elseif(isset($_POST['admit_date'])) echo $_POST['admit_date'];?>" name="admit_date">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-2 control-label" for="admit_time"><?php _e('Admit Time','hospital_mgt');?><span class="require-field">*</span></label>
						<div class="col-sm-8">
							<input id="admit_time" class="form-control validate[required] timepicker"  type="text" value="<?php if($edit){ if(isset($user_info['admit_time']))  { echo $user_info['admit_time'];} }elseif(isset($_POST['admit_time'])) echo $_POST['admit_time'];?>" name="admit_time" placeholder="H:M">
						</div>
					</div>
					<div class="form-group">
					...
					<div class="col-sm-offset-2 col-sm-8">
						...
						<a href="?dashboard=user&&page=patient&tab=addpatient_step2&action=edit&patient_id=<?php echo $patient_id;?>""><input type="button" value="<?php  _e('Back To Last Step','hospital_mgt');?>" name="back_step" class="btn btn-success margin_bottom_5px" /></a>
