  ...
  function wcfmmp_refund_requests_form_html() {
	...
	public function wcfmmp_refund_orders_actions( $actions, $vendor_id, $order, $the_order ) {
  	...
  	$order_status = sanitize_title( $the_order->get_status() );
		if( in_array( $order_status, apply_filters( 'wcfm_refund_disable_order_status', array( 'failed', 'cancelled', 'refunded', 'pending', 'on-hold' ) ) ) ) return $actions;
  	
  	if( !apply_filters( 'wcfm_is_allow_refund_requests', true ) ) return $actions;
		if( !apply_filters( 'wcfm_is_allow_paid_order_refund', false ) && ($order->withdraw_status != 'pending') && ( $the_order->get_payment_method() != 'stripe_split' ) ) return $actions;
				...
				foreach( $refund_infos as $refund_info ) {
					...
					if( $order_id ) {
						...
						if( $is_refunded ) {
							...
							if ( !empty($vendor_shipping) && isset($vendor_shipping[$vendor_id]) && $vendor_shipping[$vendor_id]['shipping_item_id'] && $product->needs_shipping() ) {
						...
						try {
							...
							if ( is_wp_error( $refund ) ) {
								...
								wcfm_log($refund->get_error_message());
							}
						} catch ( Exception $e ) {
							$wc_refund_processed = false;
							...
							if( !empty( $commission_infos ) ) {
								foreach( $commission_infos as $commission_info ) {
									...
									if( $is_partially_refunded ) {
										...
										$commission_amount = $WCFMmp->wcfmmp_commission->wcfmmp_get_order_item_commission( $commission_info->order_id, $vendor_id, $commission_info->product_id, $commission_info->variation_id, $item_total, $commission_info->quantity );
