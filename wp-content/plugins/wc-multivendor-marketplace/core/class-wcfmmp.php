  ...
  public $wcfmmp_shipping_zone;
	...
	function wcfmmp_withdrawal_periodic_scheduler_check() {
		...
		if( $withdrawal_mode && ( $withdrawal_mode == 'by_schedule' ) ) {
			wcfm_log( "PERIODIC WITHDRAWAL SCHEDULER START :: " . date( wc_date_format() . ' ' . wc_time_format(), time() ) ); // Start Log
			...
			if( !empty( $vendors ) ) {
				foreach ( $vendors as $vendor ) {
					...
					if( !$disable_vendor ) {
						...
						wcfm_log( "Periodic withdrawal start. Vendor :: " . $vendor->ID . ' Store :: ' . $shop_name );
									...
									if( $withdraw_request_id && !is_wp_error( $withdraw_request_id ) ) {
										...
										if( $is_auto_approve ) {
											...
											if( $payment_processesing_status ) {
												wcfm_log( 'Periodic withdrawal request successfully processed. Withdrawal ID :: ' . sprintf( '%06u', $withdraw_request_id ) . ' Vendor :: ' . $vendor->ID . ' Store :: ' . $shop_name );
											} else {
												wcfm_log( 'Periodic withdrawal request processing failed. Withdrawal ID :: ' . sprintf( '%06u', $withdraw_request_id ) . ' Vendor :: ' . $vendor->ID . ' Store :: ' . $shop_name );
											}
										} else {
											// Admin Notification
											...
											wcfm_log( 'Periodic withdrawal request successfully sent. Withdrawal ID :: ' . sprintf( '%06u', $withdraw_request_id ) . ' Vendor :: ' . $vendor->ID . ' Store :: ' . $shop_name );
										}
										
										do_action( 'wcfmmp_withdrawal_request_submited', $withdraw_request_id, $vendor->ID );
									} else {
										wcfm_log( 'Periodic withdrawal request failed. Vendor :: ' . $vendor->ID . ' Store :: ' . $shop_name );
									}
								} else {
									wcfm_log( "Periodic withdrawal commission less than withdrawal limit. Vendor :: " . $vendor->ID . ' Store :: ' . $shop_name );
								}
							} else {
								wcfm_log( "Periodic withdrawal no pending commission. Vendor :: " . $vendor->ID . ' Store :: ' . $shop_name );
							}
						} else {
							wcfm_log( "Periodic withdrawal payment method missing. Vendor :: " . $vendor->ID . ' Store :: ' . $shop_name );
						}
						
						wcfm_log( "Periodic withdrawal end. Vendor :: " . $vendor->ID . ' Store :: ' . $shop_name );
					}
				}
			}
			
			
			wcfm_log( "PERIODIC WITHDRAWAL SCHEDULER END :: " . date( wc_date_format() . ' ' . wc_time_format(), time() ) ); // End Log
		}
	}
