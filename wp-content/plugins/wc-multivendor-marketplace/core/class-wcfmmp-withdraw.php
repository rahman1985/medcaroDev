...
class WCFMmp_Withdraw {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_order_status_changed', array( &$this, 'wcfmmp_order_status_changed_auto_generate_withdrawal_request' ), 50, 4 );
		
		add_action( 'wcfmmp_order_item_processed', array( &$this, 'wcfmmp_order_item_auto_withdrawal_processed' ), 30, 9 ); 
	}
	 ...
	 */
	function wcfmmp_order_status_changed_auto_generate_withdrawal_request( $order_id, $status_from, $status_to, $order ) {
	...
	function wcfmmp_auto_generate_withdrawal_request( $order_id, $order_status, $processed_vendor_id = 0 ) {
		...
		if( in_array( $payment_method, apply_filters( 'wcfmmp_auto_withdrawal_exclude_payment_methods', array( 'stripe_split' ) ) ) ) return;
	...
	function wcfmmp_order_item_auto_withdrawal_processed( $commission_id, $order_id, $order, $vendor_id, $product_id, $order_item_id, $grosse_total, $total_commission, $is_auto_withdrawal ) {
		...
		if( $withdrawal_reverse ) {
			$this->wcfmmp_reverse_withdrawal_processed( $vendor_id, $order_id, $commission_id, $payment_method, $grosse_total, $total_commission, $withdraw_status, $withdraw_mode, $is_auto_withdrawal );	
		}
	}
	...
	public function wcfmmp_withdraw_status_update_by_withdrawal( $withdrawal_id, $status = 'completed', $note = '' ) {
		...
		if( !empty( $withdrawal_infos ) ) {
			...
			// Vendor Notification
			if( $vendor_id ) {
				$wcfm_messages = sprintf( __( 'Your withdrawal request #%s %s.', 'wc-multivendor-marketplace' ), '<a target="_blank" class="wcfm_dashboard_item_title" href="' . wcfm_transaction_details_url( $withdrawal_id ) . '">' . sprintf( '%06u', $withdrawal_id ) . '</a>', wcfmmp_status_labels( $status ) );
				if( $note ) {
	...
	public function wcfmmp_withdraw_status_update_by_commission( $commission_id, $status = 'completed', $note = '' ) {
		...
		if( !empty( $withdrawals ) ) {
			foreach( $withdrawals as $withdrawal ) {
				...
				// Vendor Notification
				$wcfm_messages = sprintf( __( 'Your withdrawal request #%s %s.', 'wc-multivendor-marketplace' ), '<a target="_blank" class="wcfm_dashboard_item_title" href="' . wcfm_transaction_details_url( $withdrawal->ID ) . '">' . sprintf( '%06u', $withdrawal->ID ) . '</a>', wcfmmp_status_labels( $status ) );
				if( $note ) {
					$wcfm_messages .= "<br /><b>" . __( 'Note', 'wc-multivendor-marketplace' ) . "</b>: " . $note;
				}
		...
		if( !empty( $withdrawal_infos ) ) {
			foreach( $withdrawal_infos as $withdrawal_info ) {
				...
				if( $commission_id ) {
					...
					// On withdrawal update ledge entry status update
					$WCFMmp->wcfmmp_vendor->wcfmmp_ledger_status_update( $reverse_withdrawal_id, $status, 'reverse-withdraw' );
