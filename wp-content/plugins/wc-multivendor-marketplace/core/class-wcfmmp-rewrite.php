...
class WCFMmp_Rewrites {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_layered_nav_term_html', array( $this, 'wcfmmp_layered_nav_term_html' ), 50, 4 );
	...
	function store_template_title() {
		...
		if ( !empty( $store_name ) ) {
			...
			if ( get_query_var( $this->store_endpoint('about') ) ) {
				...
				$default_qv = apply_filters( 'wcfmp_store_default_query_vars', 'products' );
	...
	function wcfmmp_layered_nav_term_html( $term_html, $term, $link, $count ) {
		if (  wcfm_is_store_page() ) {
			$author      = get_query_var( $this->wcfm_store_url );
			$seller_info = get_user_by( 'slug', $author );
			if( $seller_info && $seller_info->data->ID ) {
				...
				$store_url = wcfmmp_get_store_url( $seller_info->data->ID );
				$term_html = str_replace( $shop_url, $store_url, $term_html );
			}
		}
		return $term_html;
	}
